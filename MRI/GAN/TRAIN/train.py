...
def train(save, data_type, n_epochs=10000, n_batch=32, g_model=gen_model, d_model=dis_model, gan_model=gan_model, latent_dim=latent_dim):

    if data_type == "ADHD":
        data = readPickle("../../PICKLE_DATA/adhdImages.pkl")

    elif data_type == "CONTROL":
        data = readPickle("/home/user/Desktop/ADHD-Recognition/MRI/PICKLE_DATA/adhdImages.pkl")
    ...
    for i in range(n_epochs):
        ...
        #     plt.title(f"Epoch: {i}, D acc: {discriminator_acc} D loss: {discriminator_loss:}, G loss: {generator_loss:}")
        #     plt.show()
